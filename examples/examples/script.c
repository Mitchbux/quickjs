...
if (num>=nodeFill){ if (k->child==0){ key newk = new(key); keyInit(newk); k->child = (void*)newk;} setDAT(num-nodeFill,value,(key)k->child);  } 
else { k->data[num]=value; } }

char *getDAT(int nk, key k){
if (nk>=nodeFill){ return getDAT(nk-nodeFill, (key)k->child);} 
...
#define joinBy(by) join(here->data[vk_list], by)
#define splitToHere(in, by) sp = split(in, by, (key)malloc(sizeof(xe))); sp->parent = k; k=sp;
...
#define vk_indexer 249
#define vk_adder 250
#define vk_names 251
#define vk_label 252
#define vk_list 253
#define vk_setter 254
#define vk_getter 255

str added;
str indexed;

key ky_createModule(key k, str name, str code);
key ky_createLoader(key k, str name, str code);

#define ky_list(in) in->data[vk_names];
#define ky_add(in, item) added=apply(in->data[vk_adder], item); addxe(in->data[vk_list], added);
#define ky_set(in, name, item) /* todo ; ifKeyIsNotPresent addxe(in->data[vk_names], #name);*/ setCRC(#name, item, in->data[vk_list]);
#define ky_index(in, name, code) /* todo ; ifKeyIsNotPresent  addxe(in->data[vk_names], #name);*/ indexed=applyIdx(in->data[vk_indexer], name, code); setCRC(#name, indexed, in->data[vk_list]);
...
#define ky_delete(in,name) ky_set(in, name, 0);
#define ky_for(in, name, item) for(vk=0;vk<(in->data[vk_names]->cnt) || (name = in->data[vk_names][vk] && item=ky_find(in, name));vk++) 
#define ky_first
#define ky_next
#define ky_last
#define ky_previous
#define ky_module(in, name, code) ky_createModule(in, name, code)
#define ky_loader(in, name, code) ky_createLoader(in, name, code)

//TODO : use adapter instead of join when present 
// and recurse procedure
#define output(in) join(in, empty)

typedef str *loader (str);

str apply(any code, any stack){
    str (*loading)(str);
    loading=code;
    return loading(stack);
}

str applyIdx(any filter, any name, any code){
    str (*indexing)(str, str);
    indexing=filter;
    return indexing(name, code);
}

str ky_copyAdd(str added){
    build(xadd)
        c(" => ")
        c(added)
    shout(xadd)
...
any ky_filterIndex(str name, str code){
      
      //todo : build filter procedure 
      //compile with external 'eval'
      //str [name](str stack)
      //and return the function pointer as item

}

str ky_joinGet(key k){
    return (str) output(k);
}

str ky_nullSet(key k){
    return "";
}
...
key ky_create(str label){
    ...
    //todo : export to eval
    ky->data[vk_setter] = ky_nullSet;
    ky->data[vk_getter] = ky_joinGet;
    ky->data[vk_adder] = ky_copyAdd;
    ky->data[vk_indexer] = ky_filterIndex;
...
key ky_createModule(key k, str name, str code){
      ...
      //todo : build module indexer procedure 
      //compile with external 'eval'
      //str [name](str stack)
      //and return k with pointer to procedure 
      //attached to vk_indexer
      key newLoaderKey = ky_create(name);
      newLoaderKey->data[vk_indexer] = 0;
      ky_set(here, name, newLoaderKey)
        return here;
...
key ky_createLoader(key k, str name, str code){
      ...
      //todo : build loading procedure 
      //compile with external 'eval'
      //str [name](str added)
      //and return k with pointer to procedure 
      //attached to vk_adder
      key newLoaderKey = ky_create(name);
      newLoaderKey->data[vk_adder] = 0;
      ky_set(here, name, newLoaderKey)
        return here;
}
...
begin
    ...
    ky_set(here, Hey my own, "From CScript v px.0.08")

ky_add(here, "One") 
ky_add(here, "Two")

    console((str) joinBy(",")) line
  console((str) node(eh)) line
  console((str) node(Hey my own)) line
